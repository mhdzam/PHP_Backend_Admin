...
if (isset($_REQUEST['iSortCol_0'])) {
    if ($_REQUEST['iSortCol_0'] == 1) {
        $sort_by = " page_id " . ($_REQUEST['sSortDir_0'] === 'asc' ? 'asc' : 'desc');
    }
    ...
    if ($_REQUEST['iSortCol_0'] == 3) {
        $sort_by = " page_name " . ($_REQUEST['sSortDir_0'] === 'asc' ? 'asc' : 'desc');
    }

}

$sql_retrive0 = mysql_query("SELECT * FROM " . $prefix . "_analytics_pages where visitor_id in (SELECT visitor_id FROM " . $prefix . "_analytics_visitors where visitor_ip = '$thisip') $srch_by_nme $srch_by_id $srch_by_stts $srch_by_date1 $srch_by_date2 ");
...
if (isset($_REQUEST["sAction"]) && $_REQUEST["sAction"] == "group_action") {
    ...
    if ($multichkbx != "") {
        ...
        if ($_REQUEST["sGroupActionName"] == "b_delete") {
            ...
            $sql_delete = mysql_query("DELETE FROM  " . $prefix . "_analytics_pages WHERE page_id in ($multichkbx)");
...
$sql_retrive = mysql_query("SELECT * FROM " . $prefix . "_analytics_pages where visitor_id in (SELECT visitor_id FROM " . $prefix . "_analytics_visitors where visitor_ip = '$thisip') order by $sort_by limit $iDisplayStart,$iDisplayLength");

while ($data_retrive = mysql_fetch_array($sql_retrive)) {

    $status = 1;
    $id = $data_retrive['page_id'];
    ...
    $pg_url = $data_retrive['page_query'];
    $pg_title = $data_retrive['page_title'];
    $pg_name = $data_retrive['page_name'];
    if ($pg_title == "") {
        $pg_title = $pg_name;
    }
    if ($pg_url == "") {
    ...
    $records["aaData"][] = array(
        ...
        $event_date_from,
        '<a href="' . $pg_url . '" target="_blank">' . $pg_title . '</a>',
    );
