...
$sql_retrive0 = mysql_query("SELECT * FROM " . $prefix . "_topics_comments where wm_section_id='$wm_section' $srch_by_nme $srch_by_id $srch_by_stts $srch_by_date1 $srch_by_date2");
...
if (isset($_REQUEST["sAction"]) && $_REQUEST["sAction"] == "group_action") {
    ...
    if ($multichkbx != "") {
        ...
        if ($_REQUEST["sGroupActionName"] == "b_active") {
            $sql_active = mysql_query("UPDATE " . $prefix . "_topics_comments SET comment_status=1 WHERE wm_section_id='$wm_section' and comment_id in ($multichkbx)") or die(mysql_error
            ());

        } elseif ($_REQUEST["sGroupActionName"] == "b_block") {
            $sql_active = mysql_query("UPDATE " . $prefix . "_topics_comments SET comment_status=0 WHERE wm_section_id='$wm_section' and comment_id in ($multichkbx)") or die(mysql_error
            ());

        } elseif ($_REQUEST["sGroupActionName"] == "b_delete") {

            $sql_delete = mysql_query("DELETE FROM  " . $prefix . "_topics_comments WHERE wm_section_id='$wm_section' and comment_id in ($multichkbx)");
...
$sql_retrive = mysql_query("SELECT * FROM " . $prefix . "_topics_comments where wm_section_id='$wm_section' $srch_by_nme $srch_by_id $srch_by_stts $srch_by_date1 $srch_by_date2 order by $sort_by limit $iDisplayStart,$iDisplayLength");

while ($data_retrive = mysql_fetch_array($sql_retrive)) {

    $topic_section_is = "";

    $sql_section = mysql_query("SELECT * FROM " . $prefix . "_topics where wm_section_id='$wm_section' and topic_id ='$data_retrive[topic_id]'");
    $data_section = mysql_fetch_array($sql_section);
    $sc_nme = stripcslashes($data_section['topic_title_' . $lang]);
    $topic_section_is = "<div><small class='help-block' style='margin:0;padding:0;font-size:75%'><a href='topics.php?wm_section=$wm_section&id=$data_retrive[topic_id]&act=update'>$sc_nme</a></small></div>";
